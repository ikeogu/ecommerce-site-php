...
<?php 
		...
		while($p_price=mysql_fetch_array($run_price)){
			...
			$run_pro_price = mysql_query($con,$pro_price); 
			...
			$run_qty = mysql_query($con, $get_qty); 
			...
			$invoice = mt_rand();
				...
				$run_payment = mysql_query($con, $insert_payment); 
				...
				$run_order = mysql_query($con, $insert_order); 
				
				//removing the products from cart
				$empty_cart = "DELETE from cart";
				$run_cart = mysql_query($con, $empty_cart);
				
				
				
		if($amount==$total){
		
		echo "<h2>Welcome:" . $_SESSION['email']. "<br>" . "Your Payment was successful!</h2>";
